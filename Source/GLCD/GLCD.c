...
#define ROWS  38
#define COLUMNS 30
extern int matrix [ROWS][COLUMNS];
...
void Init_Game(void) {
    ...
    GUI_Text(80, 10, (uint8_t *) "Game Over in", White, Black); // Text on top
...
/**
 * Function to move Pacman based on joystick input.
 */
void movepacman(volatile uint16_t *pacmanX, volatile uint16_t *pacmanY, uint8_t direction) {
    ...
    if (matrix[newY][newX] != 1) {  // Ensure it's not a wall
        ...
        switch (matrix[*pacmanY][*pacmanX]) {
            case 0:  // Regular bill
                LCD_DrawBill(*pacmanX, *pacmanY, White);
                break;
    ...
    if (matrix[newY][newX] != 1) {  // Ensure it's not a wall
        ...
        if ((newY >= 21 && newY <= 23) && newX == 0) {
            newX = COLUMNS - 1;
        } else if ((newY >= 21 && newY <= 23) && newX == COLUMNS - 1) {
            newX = 0;
        }

        // Clear the old position
       LCD_DrawSquare(*pacmanX, *pacmanY, Black);
...
void UpdateCell(uint16_t x, uint16_t y) {
    switch (matrix[y][x]) {
        ...
        case 1:  // Wall
            ...
            break;
        case 3:  // Extra bill
            LCD_DrawExtraBill(x, y, White);
            break;
        default:  // Empty space
            LCD_DrawSquare(x, y, Black);
            break;
    }
}
...
void update_score(int increment) {
    ...
    GUI_Text(70, 25, (uint8_t *)scoreStr, White, Black);
}
void DrawLives(uint8_t lives) {
    uint16_t startX = COLUMNS - 3;  
    uint16_t startY = 2;           
    uint16_t spacing = 2;          
    uint8_t i;
    ...
    for (i = 0; i < 5; i++) { // Assuming a maximum of 5 lives to clear space
        LCD_Drawpacman(startX - i - spacing, startY, Black);  // Clear each previous life icon
    }

    // Draw the current lives
    for (i = 0; i < lives; i++) {
        LCD_Drawpacman(startX - i * spacing, startY, Yellow);  // Draw a life
    }
}
